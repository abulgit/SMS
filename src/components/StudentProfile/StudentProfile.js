// src/components/StudentProfile/StudentProfile.js
...
export default function StudentProfile() {
  ...
  const fetchStudent = useCallback(async () => {
    try {
      const res = await axios.get(`https://bknd-api.glitch.me/student/get/${id}`);
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      const res = await axios.put(
        `https://bknd-api.glitch.me/student/update/${id}`,
        formData,
        {
          headers: {
  ...
  const handleDelete = async () => {
    if (window.confirm("Are you sure you want to delete this student?")) {
      try {
        await axios.delete(`https://bknd-api.glitch.me/student/delete/${id}`);
            ...
            {isLoading ? (
              ...
              >
                <div className={`shadow-md rounded-lg p-8 ${darkMode ? 'bg-gray-800' : 'bg-white'}`}>
                  ...
                  <div className="flex">
                      ...
                      {["firstName", "lastName", "rollNumber", "deptName", "regNumber", "year", "age", "gender", "contactNumber", "address"].map(
                        (field, index) => (
                          <div className="flex" key={index}>
                            <label className={`w-1/3 font-semibold ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>
                              {field.charAt(0).toUpperCase() + field.slice(1).replace(/([A-Z])/g, " $1") + ":"}
                            </label>
                            <div className={`w-2/3 ${darkMode ? 'text-white' : 'text-gray-900'}`}>{student[field]}</div>
                          </div>
                        )
                      )}
                    </div>
                    ...
                    {student.profilePicture ? (
                      <img 
                        ...
                        alt="Profile" 
                        className="w-full h-80 object-cover transition-all duration-500 ease-in-out hover:border-2 hover:border-red-500"
                      />
                    ) : (
                      <div className={`w-full h-80 flex items-center justify-center transition-all duration-500 ease-in-out hover:border-2 hover:border-red-500 ${darkMode ? 'bg-gray-700' : 'bg-gray-200'}`}>
                        <span className={`text-6xl ${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>
                          {student.firstName[0]}
                        </span>
                      </div>
                    )}
                  ...
                  <div className="flex justify-end space-x-2 mt-6"><button
            ...
            <form onSubmit={handleFormSubmit} className="grid grid-cols-2 gap-4">
              {["firstName", "lastName", "rollNumber", "deptName", "year", "age", "gender", "address", "contactNumber"].map(
                ...
                <div className="col-span-1">
                  <label htmlFor="profilePicture" className={`block font-semibold mb-1 ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>
                    Profile Picture:
                  </label>
                  <input
                    type="file"
